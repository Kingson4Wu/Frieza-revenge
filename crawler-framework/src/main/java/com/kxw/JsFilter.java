...
public class JsFilter {
    ...
    public static void main(String[] args) {
        ...
        Set<String> tags = new HashSet<>(Arrays.asList("script", "link", "iframe"));
        Set<String> attrs = new HashSet<>(Arrays.asList("onclick"));

        Document doc = Jsoup.parse(str);
        String a= doc.body().html();
        ...
        for (Element ele : allElements) {
            ...
            if(tags.contains(ele.nodeName().toLowerCase())){
                ele.remove();
                continue;
            }

            for(Attribute attr: ele.attributes()){
                if(attrs.contains(attr.getKey().toLowerCase())){
